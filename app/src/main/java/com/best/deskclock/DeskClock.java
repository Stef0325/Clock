...
//    采用Lambda表达式实重写OnItemSelectedListener方法，实现AlARMS、CLOCKS、TIMERS和STOPWATCH四种功能的跳转。
...
//       配置顶部标题栏，绑定toolbar视图的实例，并将其设置成tActionBar
...
//      当actionBar不为null时，则不展示该自定义actionBar
        if (actionBar != null) {
            actionBar.setDisplayShowTitleEnabled(false);
        }

//      涉及单例模式，为toolBar设置menu及其相应的menu item
...
//        绑定fab和left_button和right_button的视图
        // Configure the buttons shared by the tabs.
        mFab = findViewById(R.id.fab);
        mLeftButton = findViewById(R.id.left_button);
        mRightButton = findViewById(R.id.right_button);

//        采用lambda表达式的写法给fab、left_button和right_button添加点击监听事件，当这些控件被点击后，
//        返回是哪个控件被点击，只有函数式接口才能使用lambda表达式
...
//        调用UI数据模型返回动画的持续时间，为200毫秒
        final long duration = UiDataModel.getUiDataModel().getShortAnimationDuration();

//     调用getScaleAnimator返回一个ObjectAnimator对象给mFab、mLeftButton和mRightButton设置显示和隐藏动画，在x轴和y轴进行缩放
...
//        采用匿名内部类的写法和适配器模式给hideFabAnimation添加动画监听器,获得监听的ValueAnimator的状态，
//        ValueAnimator四个状态：start、end、cancel、repeat，当动画执行到对应的步骤时回调对应的四个方法：
//        该代码重写了onAnimationEnd方法，在动画结束时调用该方法，更新控件的状态
...
//         mLeftButton和mRightButton同时出现，同时消失，因此只设置ｍLeftButton的监听事件，但是通知mLeftButton和
//        mRightButton动画的消失
...
//      通过建造者模式实现mFab、mLeftButton和mRightButton控件的隐藏和显示动画
//      play方法是创建播放动画的生成器对象，with方法是添加某个动画与生成器对象同时播放
...
//        采用建造者模式实现LefButton和RightButton的显示和隐藏
...
//        保持当前活动两次有3个活动存活，在左滑右划切换活动时，不用从适配器重新加载View
...
//        添加一个监听器，每当页面发生裱花或者递增滚动时，监听器会被调用
